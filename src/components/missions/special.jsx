...
const Special = ({ data, myId, fetchAccountData, loading }) => {
  ...
  // Fetch data and update state on load
  useEffect(() => {
    if (loading) {
      setTasks(data.uncomplated);
      setCompletedTasks(data.complated);
  ...
  // Save task in localStorage after the task starts
  useEffect(() => {
    if (newPost !== null) {
      const timer = setTimeout(() => {
        localStorage.setItem("task", newPost);
  ...
  // Handle the start task button click
  const handleStartClick = () => {
    const taskLinks = localStorage.getItem("link");
    setButtonState("Wait");
    setTimeout(() => {
  ...
  // Poll the task state from localStorage every 1 second
  useEffect(() => {
    const intervalId = setInterval(() => {
      setClickedMe(localStorage.getItem("task"));
    }, 1000);
  ...
  // Complete the task and refresh account data
  const checkTask = () => {
    completeTasks();
    setButtonState("Wait");
    setTimeout(() => {
              ...
              >
                <div className="task-list-image">
                  {task.image === "telegram" ? (
                    ...
                    <img src={X} className="task-list-image-icon xxx" alt="" />
                  ) : task.image === "youtube" ? (
